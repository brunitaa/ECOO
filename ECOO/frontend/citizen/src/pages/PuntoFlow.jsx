...
export default function PuntoFlow() {
    ...
    return (
      <div className="app-shell punto-success-wrap">
        <div className="punto-success">
          ...
          <p>{success.mensaje}</p>
          <p className="punto-success__detail">{success.accion?.respuesta_resumen}</p>
          <p className="punto-success__ec">
            Total estimado:{' '}
            <strong>
              {success.recompensa?.ecocoins_estimados ?? success.ecocoins_estimados} ECOO POINTS
            </strong>
          </p>
          <p className="punto-success__pending">Pendiente de aprobación por Ecoo</p>
  ...
  const ptsUnit = punto.recompensa?.puntos_por_unidad ?? estimate?.puntos_por_unidad;

  return (
    <div className="app-shell">
      <PageHeader
        title={punto.nombre}
        subtitle={punto.empresa_nombre ? `Afiliado: ${punto.empresa_nombre}` : undefined}
        backLabel="Volver"
        backTo={returnTo}
      />
      <div className="punto-hdr punto-hdr--compact">
        <span className="punto-hdr__badge">Punto Ecológico</span>
        {punto.nombre_campana && <p className="punto-hdr__campana">{punto.nombre_campana}</p>}
        {ptsUnit != null && (
          <p className="punto-hdr__rate">
            <EcoPointIcon size={22} /> {ptsUnit} ECOO POINTS por {punto.recompensa?.unidad || form.unidad}
          </p>
        )}
      </div>
        ...
        <form onSubmit={submit} className="punto-form">
          <label className="punto-form__question">
            ...
            <input
              ...
              required
            />
            {form.unidad && <span className="punto-form__unit">Unidad: {form.unidad}</span>}
          </label>
          ...
          <label className="punto-form__photo">
            ...
            <input type="file" accept="image/*" capture="environment" onChange={onPhoto} required />
            <span className="punto-form__photo-hint">Usa la cámara de tu celular en el momento</span>
          </label>

          {preview && <img src={preview} alt="Evidencia" className="punto-form__preview" />}
