          ...
          <PageTransition>
            ...
            <Link to="/escanear" className="citizen-home__scan-cta">
              ...
              >
                ...
                <p>Registra reciclaje en puntos ecológicos de empresas afiliadas</p>
              </motion.div>
              ...
              <section className="citizen-home__block">
                ...
                <ol className="citizen-home__ranking">
                  {ranking.slice(0, 5).map((r) => (
                    <li key={r.id_usuario}>
                      <span className="citizen-home__rank-pos">#{r.posicion}</span>
                      <span>{r.nombre}</span>
                      <span className="citizen-home__rank-ec">{r.ecocoins} EC</span>
                    </li>
                  ))}
                </ol>
              </section>
              ...
              <ul className="citizen-home__list">
                {acciones.length === 0 && (
                  <li className="citizen-home__empty">
                    ...
                    <p>Aún no tienes acciones. Escanea un punto ecológico para comenzar.</p>
                  </li>
                )}
                {acciones.map((a) => (
                  <li key={a.id_accion} className={`ac-row ac-row--${a.estado_validacion}`}>
                    ...
                    <motion.div className="ac-row__body">
                      ...
                      {a.respuesta_resumen && (
                        <span className="ac-row__resumen">{a.respuesta_resumen}</span>
                      )}
                      <small>{new Date(a.fecha_registro).toLocaleString('es-BO')}</small>
                      {a.estado_validacion === 'rechazada' && a.motivo_rechazo && (
                        <span className="ac-row__rechazo">Rechazada: {a.motivo_rechazo}</span>
                      )}
                    </motion.div>
